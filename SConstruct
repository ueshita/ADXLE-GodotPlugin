...
opts.Add(EnumVariable(
    ...
    "Target platform",
    host_platform,
    allowed_values=("linux", "macos", "windows", "android", "ios", "html5"),
    ignorecase=2
))
opts.Add(EnumVariable(
    "bits",
...
if env["platform"] == "linux":
    ...
    env.Append(CCFLAGS=[
        "-std=c++14", "-arch", "x86_64", "-arch", "arm64",
        "-fvisibility=hidden",
        "-ffunction-sections", "-fdata-sections",
    ...
    if env["target"] == "debug":
        ...
        env.Append(CCFLAGS=["-O3", "-std=c++14", "-fno-exceptions", "-fvisibility=hidden", "-DNDEBUG", "-s", "SIDE_MODULE=1"])
...
if env["platform"] == "windows":
    ...
    cri_libs_files = ["cri_ware_pcx64_le_import.lib"]
elif env["platform"] == "macos":
    cri_headers_path = "cri/macosx/include"
    cri_libs_path = "cri/macosx/libs"
    cri_libs_files = ["libcri_ware_macosx_LE.a"]
elif env["platform"] == "ios":
    cri_headers_path = "cri/ios/include"
    cri_libs_path = "cri/ios/libs"
    cri_libs_files = ["libcri_ware_ios_LE.a"]
elif env["platform"] == "android":
    cri_headers_path = "cri/android/include"
    cri_libs_path = "cri/android/libs/" + env["android_arch"]
    cri_libs_files = ["libcri_ware_android_LE.so"]
